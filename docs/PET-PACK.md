...
| `alert` | `{sound, desktop, push, text, title}` | 提醒（见 §4.5）；缺省 = 不提醒 |
...
| `text` | string | `null` | 通知正文，支持 `{字段名}` 插值（同 `bubble.text`） |
| `title` | string | `null` | 通知标题；缺省 = 应用名（`LitePet`） |

**通知正文的四级链**（前一级为空就落到下一级）：

| 顺序 | 来源 | 什么时候轮到它 |
|---|---|---|
| 1 | 本规则的 `alert.text` | 包作者说了算 |
| 2 | 宿主的 `note`（`agent/settled` / `agent/end` 的可选参数，`docs/PROTOCOL.md` §5） | 包没规定，宿主把这一轮的具体信息推过来 |
| 3 | 本规则的 `bubble.text` | 规则作者写的那句话本来就是给人看的 |
| 4 | 事件自带的一句话（如 `agent.settled` → 「本轮会话结束」） | 都没有 |

正文是一级一级**判空**落下来的，所以写 `"text": "{note}"`（等于“把这句话让给宿主”）在宿主沉默时会插值成空串、继续往下落，而不会把 `{note}` 字样发到用户手机上。

正文另有 **120 字上限**（按字符，超出以 `…` 收尾）：宿主推的文字与气泡文字都不受包作者控制，而手机推送把正文塞进 URL 路径，一段超长正文会变成一条发不出去的 URL。

**提醒与气泡是两回事**：`alert` 只在人**没在看屏幕**时起作用，`text` / `title` 不影响画面上的气泡。反过来，包不想被宿主的 `note` 盖住，就写自己的 `alert.text`（宿主退居第二）；写 `"text": "{note}"` 则是明确表态“听宿主的”。
