...
- **位置持久化**：`WindowEvent::Moved` 的坐标合并后写回 `config.json` 的 `x`/`y`（拖动中每秒几十条，落定 300 ms 才写一次），启动时按它摆窗口。**配置里存的一律是逻辑坐标**：`Moved` 报的是物理像素，在事件回调里就除以**窗口所在那块屏**的缩放率——缩放率随窗口被拖到哪块屏而变，事后或换个线程就查不到「当时那块屏」了（这个回调跑在主线程上，`current_monitor` 走的是内联分支，不会卡住事件循环；换算在 `src/placement.rs`）。
- ⚠️ **单位坑（两次实测）**：一是 `set_position(Physical(_))` 会先按 `window.scale_factor()` 换回逻辑坐标，而这个读在窗口刚建好时是 1.0（Retina 真值 2.0）——请求物理 300 落到物理 600，写回 600 后下次启动再翻一倍，几轮后窗口飞出屏幕。二是**存物理值这条路本身就走不通**：恢复时该除以哪块屏的缩放率无从得知，只能赌一块，赌错窗口就落到两块屏之间的空处。2026-09-22 的事故正是如此：外接屏（缩放 1.0）上记下的 `2440, -288` 被按主屏的 2.0 又除一次，窗口落到 `1220, -144`——主屏 `1470×956 @2` 与外接 `1920×1080 @1` 谁都不覆盖它，宠物照样「显示着」，但哪儿都看不见。所以恢复位置走 `LogicalPosition` 且**不做任何换算**。
- **兜底与找回**：配置里的位置若不在任何显示器上（拔过外接屏、改过分辨率），启动时改放主屏右下角并写日志；托盘菜单另有「回到主屏」，跑丢的宠物随时能拎回来。
